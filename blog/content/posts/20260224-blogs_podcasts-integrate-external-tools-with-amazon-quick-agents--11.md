...
description: 本文档旨在为第三方合作伙伴提供一份详细的实施指南，介绍如何利用**模型上下文协议（MCP）**构建新的MCP服务器，或调整现有服务器以集成**Amazon
  Quick Agents**。 以下是该集成过程的六步检查清单总结： 1. **验证 MCP 服务器架构与兼容性** * 确保您的 MCP 服务器架构符合 Amaz
