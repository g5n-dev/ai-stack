...
- 大模型
source: hacker_news
description: 在 LLM 开发中，数据质量往往比模型规模更能决定最终效果。本文记录了作者在一个下午内，通过优化数据流处理管道，显著提升 15 个主流大模型代码生成能力的实验过程。文章详细拆解了如何通过清洗和筛选高质量数据集，在不改动模型本身的前提下，实现性能的实质性飞跃。对于希望提升模型落地效果的开发者而言，这篇实战复盘提供了一套低成本、高回报的数据工程思路。
