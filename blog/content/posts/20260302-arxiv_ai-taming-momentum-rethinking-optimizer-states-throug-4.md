...
- 论文
source: arxiv
description: 现代深度学习优化器（如 Adam）依赖一、二阶动量，在提升训练效果的同时也带来了显著的显存开销。该研究提出了名为 LoRA-Pre 的低秩优化器，旨在通过低秩近似策略重构优化器状态，从而缓解显存瓶颈并保持模型性能。虽然具体的近似细节与收敛性分析无法从摘要确认，但这一思路为降低大模型训练成本提供了新的技术路径。
