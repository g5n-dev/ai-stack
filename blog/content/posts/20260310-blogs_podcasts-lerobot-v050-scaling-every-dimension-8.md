...
draft: false
entry_kind: "auto"
tags: ["LeRobot", "机器人", "Hugging Face", "端到端学习", "模型训练", "数据集", "硬件支持", "版本发布"]
...
### 如何应用到自己的项目
1.  **环境搭建：** 使用 `pip install lerobot` 安装，并配置 Isaac Sim 或 MuJoCo。
2.  **数据准备：** 使用 `lerobot-common` 工具录制或下载数据集（如 ALOHA, Droid 数据集）。
3.  **模型选择：** 根据任务复杂度选择 `act` 或 `diffusion_policy`。
4.  **训练与评估：** 使用预训练权重进行微调，利用 `python lerobot/scripts/train.py` 启动。

### 需要补充的知识
- **PyTorch 深度学习框架：** 必须熟练。
- **强化学习/模仿学习基础：** 理解 Behavior Cloning, Action Chunking 等概念。
- **ROS 2 / 硬件接口：** 如果要部署到真机，需要了解硬件通信协议。
