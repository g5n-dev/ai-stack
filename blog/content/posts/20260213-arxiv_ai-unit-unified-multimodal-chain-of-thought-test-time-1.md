...
- 论文
source: arxiv
description: 现有统一多模态模型虽能处理理解与生成任务，但往往缺乏迭代优化输出的能力，限制了其在复杂场景下的表现。本文提出的 UniT 框架通过结合代理式数据合成与统一模型训练，将测试时扩展引入多模态领域，使单一模型具备多轮推理与验证的能力。实验表明，该模型在短轨迹训练后能泛化至更长的推理链，且顺序思维链推理在效率上优于并行采样。
