...
draft: false
entry_kind: "auto"
tags: ["表征学习", "数据不平衡", "EHR", "知识嵌入", "cs.LG", "高维数据", "鲁棒性", "潜在空间"]
categories: ["论文", "数据"]
source: arxiv
description: "**标题：用于鲁棒表示学习的知识嵌入潜在投影模型** **总结：** 本文针对高维离散数据分析（如电子健康记录EHR）中常见的**数据不平衡问题**（即特征维度极高但样本量有限），提出了一种名为**知识嵌入潜在投影模型**的新方法。 **核心内容：** 1. **问题背景**：在EHR应用中，由于医疗编码系统的广泛性，"
