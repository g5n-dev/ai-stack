...
2. **风格约束**: 明确输出要求：“请使用函数式编程风格重写这段代码”或“请为这段代码生成详细的文档注释”。
...
### 提示**: 考虑在提示词中明确输出格式要求，例如指定 Claude 返回 JSON 或 CSV 结构，并包含 `original_date`, `converted_date`, `status` 三个字段。可以提供 3-5 个包含不同格式的示例（Few-shot prompting）来帮助 Claude 理解模式。
