...
description: 在这篇文章中，您将使用一个六步清单来构建新的 MCP 服务器，或对现有的 MCP 服务器进行验证和调整，以便集成到 Amazon Quick
  中。《Amazon Quick 用户指南》介绍了 MCP 客户端的行为和约束。这是一份“操作指南”，详细说明了 3P 合作伙伴需要实现哪些内容，才能通过 MCP
  与 Amazon Quick 集成。
