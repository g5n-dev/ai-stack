...
### 问题**: Matchlock 利用 Linux 的 `seccomp` 系统调用来限制 AI Agent 可以访问的系统调用。请列出 5 个你认为对于运行在沙箱中的 AI Agent 来说必须保留的基础系统调用（例如 `read` 或 `write`），并解释如果移除 `execve` 为什么能显著提高安全性。

### 提示**: 思考进程与外界交互的最基本方式（输入/输出），以及代码执行与数据读取的区别。
