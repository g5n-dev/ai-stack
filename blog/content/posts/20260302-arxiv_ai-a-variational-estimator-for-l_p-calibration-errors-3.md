...
draft: false
entry_kind: "auto"
tags: ["校准误差", "变分估计", "Lp散度", "概率预测", "模型评估", "多分类", "probmetrics", "stat.ML"]
categories: ["大模型", "数据"]
source: arxiv
description: "本文介绍了一种用于估计 $L_p$ 校准误差的新型变分方法。校准（即确保预测概率与观测类别频率一致）是机器学习系统可靠预测的基本要求。传统的校准误差评估通常基于预测值与经验频率之间的期望散度，但在多分类场景下准确估计这一指标极具挑战性。 本文提出将一种最近的变分框架进行扩展，使其不再局限于由恰当损失函数诱导的散度，而是"
