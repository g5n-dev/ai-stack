...
draft: false
entry_kind: "auto"
tags: ["LLM", "搜索相关性", "数据标注", "模型微调", "App Store", "长尾查询", "排序算法", "A/B 测试"]
categories: ["大模型", "数据"]
source: arxiv
description: "本文介绍了如何利用大语言模型（LLM）生成的数据来提升 App Store 搜索相关性的规模化应用。主要要点总结如下： **背景与挑战** 大型商业搜索系统通常依赖“行为相关性”（用户点击或下载）和“文本相关性”（语义匹配）来优化结果。然而，实际应用中面临的主要挑战是**专家标注的文本相关性数据稀缺**，而行为数据虽然"
...
**A**: 这里的“增强”主要是指增强应用商店的**排名系统**。具体来说，LLM 生成的相关性判断并不是直接用来替代现有的排序算法，而是作为“弱监督”信号或额外的训练数据。这些数据被用来训练或微调排序模型，使得模型能够更精准地识别用户意图，从而在用户搜索时将最符合需求的应用排在更靠前的位置，提升用户体验。
