...
- 数据
source: arxiv
description: 面对搜索系统中专家标注数据稀缺与长尾查询信号不足的挑战，本文探索了利用大型语言模型（LLM）生成数据以增强 App Store 排序相关性的可行性。研究发现，经过专门微调的小型模型在生成高质量标签方面优于通用大模型，并以此实现了数据规模的扩展。离线与在线实验表明，该方法能同时提升文本与行为相关性指标，为解决工业界标注瓶颈提供了新思路，但具体的模型架构细节无法从摘要确认。
