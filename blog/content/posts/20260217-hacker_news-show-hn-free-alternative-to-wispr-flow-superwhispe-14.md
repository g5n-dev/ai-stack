...
### 问题**: 构建一个基础的语音转文字命令行工具 (CLI)，能够接收本地音频文件 (如 WAV 或 MP3)，利用 OpenAI 的 Whisper API 进行转录，并将结果输出为文本文件。

### 提示**:

### 你需要熟悉如何使用 HTTP 请求库（如 Python 的 `requests` 或 `httpx`）来与 OpenAI API 进行交互。注意 API 需要的文件格式（`multipart/form-data`）以及如何处理 API 密钥的环境变量设置。
