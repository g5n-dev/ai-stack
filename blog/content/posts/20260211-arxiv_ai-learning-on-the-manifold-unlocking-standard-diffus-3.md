...
### 问题**: 在标准 DiT 架构中，通常使用可学习的类别嵌入或时间步嵌入作为条件输入。如果我们将这些简单的标量或索引替换为预训练视觉编码器（如 CLIP-ViT）提取的高维特征向量，模型在接收输入时的计算复杂度和参数量会发生什么变化？

### 提示**: 考虑 DiT 架构中 `adaLN-Zero` 或 `adaLN` 机制的工作原理，以及将低维嵌入替换为高维全局特征向量对全连接层维度的影响。
