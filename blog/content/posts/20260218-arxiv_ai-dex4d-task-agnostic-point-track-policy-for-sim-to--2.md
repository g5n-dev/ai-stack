...
- AI 工程
source: arxiv
description: 灵巧操作研究中，真实世界数据采集成本高昂，而仿真训练常受限于任务特定的环境与奖励设计。本文提出 Dex4D，这是一种任务无关的点位追踪策略，旨在通过统一的动作表示实现从仿真到真实机器人的迁移。该方法在多种操作任务中展示了零样本泛化的潜力，但具体的迁移成功率与鲁棒性细节无法从摘要确认。这一工作为构建通用型灵巧操作系统提供了新的思路，有望降低对大规模真实演示数据的依赖。
