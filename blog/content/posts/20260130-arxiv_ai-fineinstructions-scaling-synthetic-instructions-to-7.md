...
- 论文
source: arxiv
description: 现有大型语言模型普遍面临预训练目标与下游指令场景的分布差异，且高质量监督数据受限于人工标注的高成本。为此，研究提出了 FineInstructions
  方法，通过将海量无结构文档实例化为数十亿条合成指令对，尝试构建完全基于合成数据的训练范式。虽然其具体性能增益无法从摘要确认，但该工作为利用互联网规模数据弥合预训练与微调间的鸿沟提供了新思路。
