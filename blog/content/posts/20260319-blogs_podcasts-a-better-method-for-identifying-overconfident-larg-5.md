...
draft: false
entry_kind: "auto"
tags: ["过度自信", "不确定性量化", "幻觉检测", "模型评估", "LLM", "信任校准", "AI安全", "指标"]
categories: ["大模型", "AI 工程"]
source: blogs_podcasts
description: "这项研究提出了一种新的不确定性度量方法，旨在检测大语言模型（LLM）过度自信的表现。该度量基于模型对每个 token 的概率分布，计算出语义层面的不确定性得分。当得分超过预设阈值时，系统会标记潜在幻觉或错误输出，从而提示用户该答案可信度低。与传统的置信度分数不同，这种度量能够捕捉模型在语义层面的模糊与矛盾，帮助用户判断"
...
|-----|------------|
| 概率论基础 | 贝叶斯推断、概率校准理论 |
| 评估指标 | ECE、AUC-ROC、Precision-Recall |
| 可解释AI | LIME、SHAP、 probing classifiers |
| 心理学 | 元认知、置信判断偏差研究 |
