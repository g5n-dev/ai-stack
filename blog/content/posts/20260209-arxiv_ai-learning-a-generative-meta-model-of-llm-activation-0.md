...
- 论文
source: arxiv
description: 针对大型语言模型内部激活的复杂分布特性，本文提出了一种生成式元模型框架，旨在通过学习模型激活的潜在分布来捕捉其内在特征。作者通过在特定数据集上的训练与验证，展示了该模型在生成与真实激活高度相似的样本方面的能力，从而为理解模型内部表征提供了新的视角。然而，由于摘要未提供具体的消融实验细节，目前无法从摘要确认该方法的泛化能力及在不同规模模型上的表现差异。
