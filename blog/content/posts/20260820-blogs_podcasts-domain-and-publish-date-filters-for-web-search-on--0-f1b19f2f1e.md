...
first_seen_at: 2026-08-20T03:58:42.230123Z
...
interpretation_sha256: "sha256:e2b834f2da629c65c6ba6e1dbe2448c1a385e10b11ae22a77b3cbf1121221a3e"
description: "AgentCore 的 Web Search 连接器在 1.2.0 版本中加入了运行时域名过滤和发布‑日期过滤，开发者可以在每次调用时指定仅返回特定来源或特定时间范围内的搜索结果，且过滤在服务器端完成。"
...
last_seen_at: 2026-08-20T03:58:42.230123Z
---
...
### 这是什么  
AgentCore 的 Web Search 连接器在 1.2.0 版本中加入了运行时域名过滤和发布‑日期过滤，开发者可以在每次调用时指定仅返回特定来源或特定时间范围内的搜索结果，且过滤在服务器端完成。

### 用在哪里  
适合需要严格控制信息来源的业务场景，例如合规审查（仅限授权站点）、实时产品信息查询（仅最新数据）或多租户平台（各租户可动态设定不同过滤规则）。

### 可以推断的  
推测：运行时过滤只能缩小管理员预设的搜索范围，无法突破已有的域名禁令，确保企业级治理始终生效。  
推测：搜索请求始终在 AWS 区域内处理，理论上能够满足对数据跨境有合规要求的组织。
