...
draft: false
entry_kind: "auto"
tags: ["3D生成", "分词方法", "语义分词", "自回归", "计算机视觉", "生成模型", "特征对齐", "3D形状"]
categories: ["论文", "AI 工程"]
source: arxiv
...
1. **类别依赖**：语义显著性的排序在 ShapeNet 上进行，跨到非结构化或高度变形的类别（如自然场景）时排序可能失效。  
2. **DINO 预训练偏差**：语义特征来源于 2D 图像模型，对 3D 几何细节（如细小孔洞）可能不够敏感。  
3. **长序列生成**：当目标形状极其复杂时，仍需相对较长的 token 序列，极端情况下 token 压缩效果可能不如预期。  
...
**实施步骤**:
1. 对所有点云或网格进行质心对齐，使形状的中心位于坐标原点。  
2. 采用最大欧氏距离或包围盒对形状进行尺度归一化，使所有形状的最大维度为 1（或统一到指定尺度）。  
3. 如有必要，使用主成分分析（PCA）确定主要朝向并进行姿态对齐，或使用手动标注的姿态标签进行刚性变换。  
4
