...
draft: false
entry_kind: "auto"
tags: ["Nova Forge", "AWS", "VOC分类", "数据混合", "模型评估", "垂直领域模型", "开源模型", "客户之声"]
categories: ["大模型", "数据"]
source: blogs_podcasts
description: "**总结：AWS中国应用科学团队关于Nova Forge在客户语音分类任务中的评估结果** 在本文中，AWS中国应用科学团队分享了关于Nova Forge（一种通过数据混合构建专业AI的方法）的综合评估结果。该评估基于一项极具挑战性的客户语音分类任务，并与开源模型进行了基准测试。核心发现是Nova Forge能够在保持"
...
**如何应用到自己的项目**
1.  **数据盘点**：将现有数据分为"通用数据"（如Wiki、Books）和"领域数据"（如公司文档、日志）。
2.  **基座选择**：选择一个智能水平较高且开源许可友好的基座模型（如Llama 3或Mistral）。
3.  **混合实验**：设计多组实验，例如 9:1, 8:2, 5:5 的通用与领域数据混合比例，观察Loss收敛情况和验证集效果。

**行动建议**
*   不要直接在原始数据上微调。
*   构建自动化评估流水线，实时监控"遗忘"现象。
*   关注数据的质量而非数量，清洗是必须的。

**需补充的知识**
*   学习**课程学习**的理论。
*   掌握**参数高效微调（PEFT）**技术（如LoRA），以降低训练成本。
