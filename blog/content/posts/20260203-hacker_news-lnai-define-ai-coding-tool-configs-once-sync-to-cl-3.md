...
1.  **兼容性压力测试（指标）**：
    选取 5 款主流 AI 编程工具（Cursor, Windsurf, Copilot, Claude Dev, Continue），配置同一套包含复杂逻辑（如多角色切换、特定输出格式要求）的 LNAI 配置文件。观察是否所有工具都能无损执行，记录“功能缺失率”和“格式错误率”。
...
### 问题**: 假设你需要为 LNAI 设计一个最基础的配置文件格式，用于定义“系统提示词”这一项配置。请设计一个 JSON 结构，使其能同时被 Cursor 和 Claude 读取。

### 提示**: 考虑通用字段名。Cursor 通常使用 `system_prompt`，而 Claude 可能期望 `systemMessage` 或类似的键。你的结构如何兼容这两种命名习惯？
