...
- 论文
source: arxiv
description: 针对自主智能体训练中环境匮乏的扩展瓶颈，本文提出了 Agent World Model (AWM)，一个基于代码驱动的全合成环境生成管道。该研究构建了覆盖日常场景的千级环境，通过高可靠性的状态转换与可执行的数据库设计，支持智能体进行大规模强化学习训练。实验表明，该方法能有效提升智能体的分布外泛化能力，为低成本、高可控的智能体训练提供了新范式，但具体的算法实现细节无法从摘要确认。
