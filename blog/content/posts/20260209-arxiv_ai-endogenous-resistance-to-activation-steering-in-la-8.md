...
- 安全
source: arxiv
description: 本文探讨了大语言模型在推理过程中自发抵抗激活引导干预的现象，即“内生性引导抵抗”（ESR）。作者通过实验证实，即便面对与任务意图冲突的激活干预，模型仍能表现出显著的鲁棒性，这挑战了人们对模型内部表征可操纵性的既有认知。虽然其背后的具体神经机制尚无法从摘要确认，但该发现为理解模型的安全边界及激活干预的局限性提供了新的实证依据，或将对未来的对齐研究与鲁棒性评估产生深远影响。
