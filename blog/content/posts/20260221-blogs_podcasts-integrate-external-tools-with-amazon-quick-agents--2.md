...
description: 本文介绍了如何利用模型上下文协议（MCP）将外部工具与 Amazon Quick Agents 进行集成。文章旨在为第三方合作伙伴提供一份实施指南，通过六步检查清单来指导用户构建新的
  MCP 服务器，或对现有的服务器进行调整和验证，以满足 Amazon Quick 的集成要求。同时，文中还引用了 Amazon Quic
