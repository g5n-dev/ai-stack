...
### 问题**: 在本地环境中，使用 Ollama 或 LM Studio 部署一个开源大模型（如 Llama 3 或 Mistral），并通过命令行（curl）发送一个简单的测试请求，验证模型是否正常运行。

### 提示**: 检查本地模型服务的默认端口（Ollama 通常为 11434），并构造一个包含 `model` 和 `prompt` 字段的 JSON 请求体。
