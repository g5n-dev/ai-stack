...
- 后端
source: arxiv
description: 针对现有基于大语言模型（LLM）的单元测试生成技术在处理复杂方法时面临的覆盖不足与上下文长度限制问题，本文提出了一种通过消除已覆盖代码来优化测试生成的自动化方法。该方法旨在通过精简输入上下文来提升生成效率，但具体的算法细节与消融实验结果无法从摘要确认。这一工作为解决大模型在长代码序列理解中的局限性提供了新的思路，有望提升自动化测试生成的实用性与覆盖率。
