...
description: '**总结：** 亚马逊 Bedrock 现已提供 NVIDIA Nemotron 3 Nano 模型。这是一种完全托管的无服务器模型，继此前支持
  Nemotron 2 Nano 系列模型后的又一更新。本文介绍了该模型的技术特性、潜在应用场景，并提供了在 Amazon Bedrock 环境中使用该模型构建生成式
  AI 应'
