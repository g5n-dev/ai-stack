...
- 论文
source: arxiv
description: 高效训练大型语言模型（LLM）是现代机器学习的核心挑战。此前提出的 POET 框架虽能通过正交等价变换优化权重矩阵，却受限于高昂的内存与计算开销。为此，研究者提出了可扩展的变体
  POET-X，旨在以显著降低的计算代价执行正交等价变换。该方法在继承原有泛化能力与训练稳定性的同时，大幅提升了吞吐量与内存效率，实现了在单块 Nvidia H100
  GPU 上对十亿级参数 LLM 的预训练。
