...
- 论文
source: arxiv
description: 本文针对大语言模型在数学推理中误用引理的问题，提出了名为 RULES 的方法。该方法通过资格推理和章节感知的强化学习，旨在提升模型对引理使用的可靠性。由于摘要信息不完整，具体的算法细节和实验效果无法从摘要确认。该研究有望为增强大模型在复杂定理证明中的逻辑严谨性提供新的思路。
  本文介绍了一种名为 RULES 的新方法，旨在解决大型语言模型（LLM）在数学推理中错误引用引理（Lemma）的问题。
