...
first_seen_at: 2026-08-16T11:34:07.582072Z
...
interpretation_sha256: "sha256:481828a9d33f2f2dd0a044d163d8460cfb881b96fa664cd3ab2ca346c1a96087"
description: "Flue 2 是由 Astro 框架创始人 Fred Schott 推出的 agent 框架，通过在每一次模型调用前重新渲染的 React 风格 Hooks，让 agent 能够动态管理状态、监听生命周期事件并按需挂载不同的工具或子 agent。"
external_url: https://www.latent.space/p/flue-2
parent_observation_id: null
last_seen_at: 2026-08-16T11:34:07.582072Z
---
...
### 这是什么
Flue 2 是由 Astro 框架创始人 Fred Schott 推出的 agent 框架，通过在每一次模型调用前重新渲染的 React 风格 Hooks，让 agent 能够动态管理状态、监听生命周期事件并按需挂载不同的工具或子 agent。

### 用在哪里
适用于需要构建实时响应、可组合工具且行为随对话进程变化的 AI 应用，如客服机器人、工作流自动化或需要 agent 在运行期间自行获取额外资源的场景。

### 可以推断的
推测：受 Flue 2 “以 harness 为核心” 思路影响，后续新出现的 agent 框架可能会把内置执行环境和资源抽象列为默认特性。  
推测：当前 agent 开发仍处于早期探索阶段，框架接口和抽象模型会频繁迭代，使用者在评估时需关注社区活跃度与文档更新速度。
