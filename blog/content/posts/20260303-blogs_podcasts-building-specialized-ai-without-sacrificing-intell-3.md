...
draft: false
entry_kind: "auto"
tags: ["Nova Forge", "AWS", "VOC分类", "数据混合", "模型评估", "开源模型", "基准测试", "垂直领域"]
categories: ["大模型", "数据"]
source: blogs_podcasts
description: "以下是基于您提供的标题和摘要进行的简洁中文总结（注：因原文仅提供了标题和简短摘要，总结主要基于此核心信息展开）： **总结：打造兼顾智能与专业化的AI——Nova Forge实战评估** 本文分享了AWS中国应用科学团队的一项评估成果，重点展示了**Nova Forge**技术在构建人工智能模型时的核心优势：即在打造*"
...
**为什么这个观点重要**
这一观点对企业级 AI 落地至关重要。企业在构建私有化模型时，最大的痛点是：开源模型很聪明但不懂业务，微调后的模型懂业务但变“笨”了（例如只会输出 JSON，无法正常对话）。Nova Forge 提供了一条路径，让企业能够获得既懂业务逻辑又保持高智商的 AI 员工。
...
**如何应用到自己的项目**
1.  **建立评估基准:** 在微调前，先选定一套通用能力测试题（如 reasoning 能力）和领域测试题。
2.  **数据分层:** 将数据分为 `General Capabilities`（通用能力）、`Domain Knowledge`（领域知识）、`Format/Style`（格式与风格）。
3.  **小规模实验:** 不要上来就全量训练。用 1/10 的数据，尝试不同的混合比例（如 9:1, 8:2, 5:5），观察 Loss 曲线和评估结果。
4.  **监控遗忘:** 重点观察模型在微调后，是否还能回答“你好”、“1+1等于几”等通用问题。

**具体的行动建议**
*   如果你的模型变傻了，增加通用指令数据的比例。
*   如果你的模型不懂业务，增加领域问答数据的比例。
*   使用像 Nova Forge 这样的自动化工具来辅助寻找最佳比例，而不是手动猜测。
...
**立场与验证方式**
我**支持**该命题。数据混合是当前解决 Catastrophic Forgetting 最有效的方法之一。
*   **验证方式:** 复现实验。选取 Llama-3-8B 作为基座，准备 VOC 数据集和通用指令集（如 OpenOrca
