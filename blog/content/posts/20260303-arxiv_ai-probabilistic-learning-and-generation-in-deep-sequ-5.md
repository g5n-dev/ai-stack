...
- 论文
source: arxiv
description: 深度序列模型虽预测性能优异，但常因缺乏不确定性感知而限制了其实际部署。本文旨在探讨如何将概率模型（特别是贝叶斯方法）引入深度序列框架，以利用概率规则量化未观测变量的不确定性。针对大规模应用中精确贝叶斯推断计算不可行的问题，作者可能提出或评估了相应的近似推断策略。尽管具体技术细节无法从摘要确认，该研究有望为提升序列模型在复杂环境下的鲁棒性与安全性提供理论支持。
