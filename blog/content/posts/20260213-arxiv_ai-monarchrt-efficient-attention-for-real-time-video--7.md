...
- 系统与基础设施
source: arxiv
description: MonarchRT 针对实时视频生成中扩散 Transformer 计算成本高昂的瓶颈，提出了一种基于 Monarch 矩阵分解的新型结构化注意力机制。作者指出，视频注意力并非单纯的稀疏结构，而是由时空位置主导的周期性与动态语义混合而成，这使得传统稀疏近似方法在少步自回归场景下失效。MonarchRT
  通过特定的块结构对齐与平铺参数化，在保持计算效率的同时捕捉复杂的时空依赖。
