...
- AI 工程
source: juejin
description: 这是一份关于“AI 对话应用中 JS 流式接口数据处理”的简洁总结： **核心概念：流式传输** AI 聊天应用采用流式传输，本质上是**边生成边传输**。不同于传统请求需等待全部内容生成完毕才返回，流式处理将数据像“水龙头滴水”一样，实时分块推送到前端。
  **前端处理流程（JavaScript 逻辑）：** 1. *
