...
- 大模型
source: arxiv
description: '**AgentFactory 概述** AgentFactory 是一个面向 LLM‑Agent 的自演进框架。与以往把成功经验记录为文字
  Prompt 或反思不同，它把每一次成功解决的任务封装为**可执行的 Python 子代理代码**。这些子代理以纯 Python 形式保存，并配有统一的标准文档，因而具备**跨平台'
