...
draft: false
entry_kind: "auto"
tags: ["视频生成", "零样本学习", "相机控制", "3D运动控制", "计算机视觉", "生成式AI", "动作控制", "视频模型"]
categories: ["论文", "大模型"]
source: arxiv
description: "背景 视频生成在艺术创作中需要同时控制演员动作和摄像机轨迹，实现细粒度的表演与镜头调度。 方法 ActCam 是一种零样本方法，基于任意预训练的图转视频扩散模型（接受深度和姿态条件）。给定驱动视频和目标相机运动，ActCam 生成逐帧几何一致的姿态与稀疏深度条件。采用两阶段调节策略：去噪早期同时利用姿态和稀疏深度约束场"
