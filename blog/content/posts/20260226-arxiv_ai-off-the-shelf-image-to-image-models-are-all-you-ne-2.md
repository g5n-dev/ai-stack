...
- 大模型
source: arxiv
description: 本文探讨了现有图像保护方案在通用图像生成模型面前的脆弱性。研究利用现成的图像到图像迁移模型构建攻击框架，成功瓦解了当前的防御机制。虽然摘要未详述具体算法细节，但该工作揭示了现有保护方案在对抗生成式攻击时的结构性漏洞。这一发现表明，未来的图像防护研究需重新评估基于生成模型的对抗风险，并探索更具鲁棒性的防御范式。
