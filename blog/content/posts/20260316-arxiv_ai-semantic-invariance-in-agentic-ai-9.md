...
- 论文
source: arxiv
description: 针对大语言模型在自主推理任务中面临的关键稳定性挑战，本文探讨了“语义不变性”这一命题，即模型在面对语义等价但表述不同的输入时应保持输出一致。为此，作者构建了一个包含多种语义转换的变形测试框架，并对跨越四个架构家族的七个基础模型进行了系统性评估。虽然摘要未完整披露具体结论，但该研究为量化智能体在复杂科学推理中的鲁棒性提供了新的基准视角，有助于推动更可靠的自主代理系统的发展。
