...
first_seen_at: 2026-08-20T22:45:12.890992Z
...
interpretation_sha256: "sha256:f7c2767e7580f509c4538e2103c50931f79dd06ff9f90235b007a1d96bd8c802"
description: "Amazon Bedrock 为 GPT‑5.6（Sol、Terra、Luna）提供跨区域推理能力，通过推理配置文件在多个 AWS 区域之间路由请求，旨在扩大可用算力、提升吞吐量并保持性能稳定。"
...
last_seen_at: 2026-08-20T22:45:12.890992Z
---
...
### 这是什么  
Amazon Bedrock 为 GPT‑5.6（Sol、Terra、Luna）提供跨区域推理能力，通过推理配置文件在多个 AWS 区域之间路由请求，旨在扩大可用算力、提升吞吐量并保持性能稳定。

### 用在哪里  
适用于在单一区域算力不足、需跨区域弹性扩展、或对数据处理地域有合规要求的开发者和服务，尤其在使用 OpenAI Responses/Chat Completions 或 Bedrock Converse API 时。

### 可以推断的  
推测：跨区域推理主要解决容量瓶颈，而非直接降低响应延迟或成本。  
推测：全局配置文件会把请求分发至所有符合条件的商业区域，若业务对数据主权有严格限制，需要选择对应的地域配置文件或直接调用单区域模型。
