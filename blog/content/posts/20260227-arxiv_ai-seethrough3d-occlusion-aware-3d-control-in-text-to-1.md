...
- 论文
source: arxiv
description: 本文介绍了 **SeeThrough3D**，一种针对文本生成图像任务的新型模型，旨在解决现有方法在处理 3D 布局条件生成时常常忽略的**遮挡推理**问题。
  **核心问题** 现有方法虽然能根据输入布局生成逼真场景，但在处理物体间的精确遮挡关系时往往表现不佳，难以生成具有深度一致几何形状和比例的部分遮挡物体。 **解
...
SeeThrough3D 是一种针对文本到图像生成模型的 3D 控制技术。它主要解决了现有的基于 2D ControlNet 或简单 3D 条件的方法在处理复杂场景时的“遮挡感知”缺失问题。
...
### SeeThrough3D 的核心技术原理是什么？它是如何实现 3D 控制的？

SeeThrough3D 的核心在于构建了一个“Occlusion-Aware”（遮挡感知）的生成流程。它并非简单地输入一张 3D 模型的渲染图，而是采用了以下策略：
...
### 与 ControlNet 或其他基于 3D 的生成方法相比，SeeThrough3D 有什么优势？

相比于现有方法，SeeThrough3D 的主要优势体现在对复杂空间关系的处理能力上：
...
### 使用 SeeThrough3D 需要什么样的输入数据？

根据 arxiv 论文的典型设置，使用 SeeThrough3D 通常需要以下输入：
...
### SeeThrough3D 可以用于生成视频或 3D 资产吗？

虽然 SeeThrough3D 主要针对 Text-to-Image（文本生成图像）任务，但其技术原理对相关领域有启发性：
...
尽管 SeeThrough3D 在遮挡处理上取得了突破，但仍存在一些潜在局限性：
