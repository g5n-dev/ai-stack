...
first_seen_at: 2026-08-16T13:00:39.072793Z
...
interpretation_sha256: "sha256:540d5c52c0cb1d3687cc7e735f20a5681b08bf4ce217b186b85a374ef0aded31"
description: "QuoteBench 是一个用于评估大模型生成 Bash 命令后、在序列化、封装或重新解析等环节引入路径失败的基准。它通过对比生成阶段的匹配分数与执行后的真实最终状态，揭示匹配得分可能掩盖的实际错误。"
external_url: http://arxiv.org/abs/2608.13547v1
parent_observation_id: null
last_seen_at: 2026-08-16T13:00:39.072793Z
---
...
### 这是什么  
QuoteBench 是一个用于评估大模型生成 Bash 命令后、在序列化、封装或重新解析等环节引入路径失败的基准。它通过对比生成阶段的匹配分数与执行后的真实最终状态，揭示匹配得分可能掩盖的实际错误。

### 用在哪里  
适合从事大模型编码代理研发和测评的研究者或工程师，用于评估模型在真实命令行环境中的鲁棒性，并提醒在报告中应明确模型配置、生成契约、执行路径等信息。

### 可以推断的  
推测：在仅依赖匹配分数的情况下，模型的真实错误率可能被低估。  
推测：边界适应能力较强的模型在面对解析层干扰时，能够更快恢复成功率。
