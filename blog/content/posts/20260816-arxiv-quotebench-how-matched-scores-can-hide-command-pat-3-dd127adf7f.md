...
first_seen_at: 2026-08-16T15:35:20.983343Z
...
interpretation_sha256: "sha256:56f58edd0c5082fc7d3953d3c2e56f930137b908c23a6c686de82474fd5e02f1"
description: "QuoteBench 通过在 56 项一次性任务和 14 类事故衍生家族上对比模型生成的原始命令路径与解析后执行路径的最终状态，量化了仅凭匹配得分无法区分生成错误与解析引入的错误这一现象。"
external_url: http://arxiv.org/abs/2608.13547v1
parent_observation_id: null
last_seen_at: 2026-08-16T15:35:20.983343Z
---
...
### 这是什么  
QuoteBench 通过在 56 项一次性任务和 14 类事故衍生家族上对比模型生成的原始命令路径与解析后执行路径的最终状态，量化了仅凭匹配得分无法区分生成错误与解析引入的错误这一现象。  

### 用在哪里  
适用于需要评估 LLM 生成脚本在实际系统接口中执行可靠性的研发团队，尤其是关注命令生成与执行路径差异对成功率影响的场景。  

### 可以推断的  
推测：在实际部署中，若不披露生成契约与执行传输的边界信息，即使模型得分很高，也可能隐藏后续解析导致的大幅成功率下降。  
推测：让模型在已知解析边界的情况下调整生成策略，可能会在原有基础上显著提升任务完成率。
