...
- 数据
source: arxiv
description: 针对无监督表征学习中语义可解释性与模型稳定性难以兼顾的问题，本文提出了熵序流框架。该方法借鉴 PCA 的思想，依据潜在变量解释的熵进行排序，从而在推理阶段灵活分离出核心语义表征与细节噪声。实验表明，EOFlows
  在图像数据上实现了高压缩率与强去噪能力，但其理论收敛性及在非视觉数据上的泛化表现无法从摘要确认。
