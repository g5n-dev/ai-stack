...
date: 2026-08-19T05:40:40+08:00
draft: false
entry_kind: "auto"
...
first_seen_at: 2026-08-18T21:38:37.667458Z
...
interpretation_sha256: "sha256:b5e0c5e3471642bc201567c2947b042e1b467e58dedc117bbf08c8e4f2bc020f"
description: "该工作提出一种名为 BATON 的框架，旨在解决机器人长时域操作中子任务之间的误差累积和状态不匹配问题。它通过把每个子任务作为独立的探索单元，并在记忆中加入对过渡条件的感知，使语言模型负责整体规划，而仅在需要接触的动作阶段调用视觉‑语言‑动作模型，从而在保持模型参数不变的情况下提升成功率。"
external_url: http://arxiv.org/abs/2608.16889v1
parent_observation_id: null
last_seen_at: 2026-08-18T21:38:37.667458Z
---
...
### 这是什么  
该工作提出一种名为 BATON 的框架，旨在解决机器人长时域操作中子任务之间的误差累积和状态不匹配问题。它通过把每个子任务作为独立的探索单元，并在记忆中加入对过渡条件的感知，使语言模型负责整体规划，而仅在需要接触的动作阶段调用视觉‑语言‑动作模型，从而在保持模型参数不变的情况下提升成功率。  

### 用在哪里  
适用于需要将多个接触密集动作串联执行的机器人操作任务，例如多阶段抓取、装配或移动操作。该方案对关注长时域任务规划、模块化机器人学习系统的研究者和工程师具有直接参考价值。  

### 可以推断的  
- 推测：子任务解法被缓存后可在不同任务中复用，从而降低新场景的探索成本。  
- 推测：过渡感知的验证步骤虽然会带来额外的计算开销，但有助于避免因前置子任务残差导致的失败。
