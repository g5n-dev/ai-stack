...
- 论文
source: arxiv
description: 本文探究了语言模型在处理区分性论元标记（DAM）时表现出的类型学倾向，旨在揭示模型是否习得人类语言中基于语义显著性的形态标记规律。研究通过GPT-2在包含不同DAM系统的合成语料库上进行受控训练，发现模型虽能习得“显性标记针对非典型论元”的自然倾向，却未能复现人类语言中宾语标记强于主语的偏好。这一结果暗示，不同的类型学规律可能源自不同的认知或机制来源，尽管具体成因尚无法从摘要确认。
