...
### 如何应用到自己的项目
1.  **评估数据缺口**：分析现有模型在哪些场景或意图下表现不佳（如讽刺、反问）。
2.  **构建人设库**：针对缺口，设计10-20个详细的虚拟人设。
3.  **利用强模型生成**：使用GPT-4或Claude 3等强模型，通过Prompt Engineering让它们扮演这些角色进行对话。
4.  **清洗与入库**：人工抽检生成质量，合格后加入训练集。
