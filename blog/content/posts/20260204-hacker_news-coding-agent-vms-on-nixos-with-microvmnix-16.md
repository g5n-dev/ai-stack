...
draft: false
entry_kind: "auto"
tags: ["NixOS", "Microvm", "Coding Agent", "虚拟机", "DevOps", "自动化", "基础设施", "Linux"]
categories: ["开发工具", "系统与基础设施"]
source: hacker_news
description: "随着自动化工具的演进，能够独立编写代码的 Agent 正在改变开发者的工作流。本文探讨了如何在 NixOS 环境下利用 Microvm.nix 构建隔离的虚拟机，从而为这些智能体提供安全且可复现的执行环境。通过阅读这篇文章，你将掌握一套轻量级的沙箱方案，既能高效运行 AI 生成的代码，又能确保宿主系统的稳定性与安全性。"
...
### 问题**: 在使用 Microvm.nix 时，默认情况下虚拟机（VM）的根文件系统是易失性的。请配置一个 MicroVM，使其在重启后能够保留 `/var/lib` 目录下的数据（例如数据库文件），同时保持根文件系统其他部分的不可变性。

### 提示**: 考虑使用 Microvm.nix 提供的 `shares` 或 `mounts` 选项。你需要将宿主机上的一个特定目录挂载到虚拟机内的 `/var/lib`，并确保该挂载点具有读写权限。
