...
### 问题**: 尝试修改 LangBot 的系统提示词，使其在回答问题时强制采用某种特定的人物设定（例如：一位严厉的代码审查员或一位只会用押韵句说话的诗人）。观察模型在多轮对话中是否能始终保持该设定。

### 提示**: 关注初始化客户端或发送消息请求时传入的 `system_message` 或 `messages` 数组中的第一条内容。思考如何通过指令明确界定角色的行为边界。
