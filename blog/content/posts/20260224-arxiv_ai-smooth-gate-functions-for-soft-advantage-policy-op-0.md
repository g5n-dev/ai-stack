...
draft: false
entry_kind: "auto"
tags: ["强化学习", "GRPO", "SAPO", "策略优化", "LLM训练", "平滑门函数", "模型稳定性", "RLHF"]
categories: ["大模型", "论文"]
source: arxiv
description: "以下是对该内容的中文总结： 本文提出了一种用于大语言模型（LLM）训练的**平滑门控函数**，旨在解决现有强化学习算法中的不稳定性问题。 **背景与动机：** 群体相对策略优化（GRPO）虽然在提升大模型推理能力方面表现出色，但其使用的“硬裁剪”机制容易导致训练过程出现不稳定。虽然软自适应策略优化（SAPO）通过引入基"
...
### 主要结果
1.  **收敛速度与稳定性**：实验结果显示，使用平滑门控函数（特别是优选的函数族）的方法，其训练Loss曲线下降更平滑，没有出现GRPO常见的剧烈尖峰。
2.  **最终性能**：在数学推理的准确率上，提出的方法达到了与GRPO相当甚至略优的水平，证明了“平滑”并未以牺牲性能为代价。
3.  **函数对比**：作者可能发现 **Tanh** 或 **Swish** 类型的门控函数在某些场景下表现优于 Sigmoid，因为它们在0点附近的线性特性或对称性更好地匹配了优势值的分布（正负优势均重要）。
...
### 实际应用场景
1.  **复杂推理模型训练**：用于训练 O1 系列类的思维链模型，这类模型极度依赖强化学习来探索长推理路径，训练过程中的稳定性至关重要。
2.  **RLHF/RLAIF 流程**：在基于人类反馈或AI反馈的对齐阶段，替代传统的PPO或GRPO，减少训练失败（Reward Hacking）的风险。
