...
draft: false
entry_kind: "auto"
tags: ["文本检索", "域适应", "密集检索", "采样策略", "强化学习", "Inf-DDS", "嵌入模型", "信息检索"]
categories: ["论文", "数据"]
source: arxiv
description: "以下是针对该论文内容的中文总结： **论文标题：** 基于引导采样的文本检索器域适应方法 **核心问题：** 通用的开放域密集检索系统通常使用混合了大量不同语料库和搜索任务的数据进行训练。然而，现有的数据采样策略（如均匀采样、按比例采样或依赖人工专家监督）往往效率低下或成本高昂，如何找到最优的训练数据采样策略以提升模型"
