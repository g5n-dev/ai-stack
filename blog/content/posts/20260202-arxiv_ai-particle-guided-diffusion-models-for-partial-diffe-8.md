...
- 大模型
source: arxiv
description: 针对偏微分方程（PDE）的求解问题，本文提出了一种基于粒子引导扩散模型的新方法，通过将物理残差与观测约束融入采样过程，确保生成结果符合物理规律。该方法构建了基于顺序蒙特卡洛（SMC）的计算框架，并在基准测试中显示出优于现有生成方法的数值精度。然而，该方法的计算成本与对复杂边界条件的鲁棒性无法从摘要确认，未来在多物理场耦合系统中的应用值得关注。
