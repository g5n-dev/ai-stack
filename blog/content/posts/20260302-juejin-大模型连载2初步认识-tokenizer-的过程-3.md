...
draft: false
entry_kind: "auto"
tags: ["Tokenizer", "Token", "分词器", "自然语言处理", "LLM", "文本生成", "模型基础", "AI聊天"]
categories: ["大模型", "AI 工程"]
source: juejin
description: "由于提供的内容在“像素之”处截断，以下总结基于文本中已呈现的完整部分进行概括： 本文是大模型连载系列文章的第二部分，旨在介绍 **Tokenizer（分词器）** 的初步概念与作用。 1. **核心概念回顾**： 文章首先承接上一节内容，重申了 **Token（词元）** 的定义。在自然语言处理（如文本生成、AI聊天）"
