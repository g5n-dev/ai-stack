...
first_seen_at: 2026-08-14T18:06:08.925156Z
...
interpretation_sha256: "sha256:11937fb939dd11e6e0a15a4a8ee8ed823bddb54327a940ea098ea2a25a36e210"
description: "QuoteBench 是一个用于测评大语言模型在生成 Bash 命令后、后续解析和传输环节中出现的错误是否能被常规匹配得分掩盖的基准。它通过在执行路径中加入一个未转义的额外解析器，衡量生成与执行之间的性能差异。"
external_url: http://arxiv.org/abs/2608.13547v1
parent_observation_id: null
last_seen_at: 2026-08-14T18:06:08.925156Z
---
...
### 这是什么  
QuoteBench 是一个用于测评大语言模型在生成 Bash 命令后、后续解析和传输环节中出现的错误是否能被常规匹配得分掩盖的基准。它通过在执行路径中加入一个未转义的额外解析器，衡量生成与执行之间的性能差异。

### 用在哪里  
适用于需要验证 LLM 代码生成在实际命令执行链路中可靠性的研究者和工程师，特别是关注脚本自动化或 AI 编程助手的团队。

### 可以推断的  
推测：仅凭匹配得分评估模型容易低估其在真实部署环境中的失败率。  
推测：更完整的评估应包括模型配置、执行路径和最终状态验证等信息，而不只是表面分数。
