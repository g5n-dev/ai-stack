...
date: 2026-08-15T04:48:41+08:00
draft: false
entry_kind: "auto"
...
first_seen_at: 2026-08-14T20:45:21.287190Z
...
interpretation_sha256: "sha256:05d4be06b90016466c31117e21653b9ab888cf5571cce9adc7447b79d94d5ef7"
description: "QuoteBench 是一个测评框架，通过在命令生成与执行传输之间加入未转义的解析器，利用精确的最终状态校验来区分模型生成的错误与后处理引入的故障。"
external_url: http://arxiv.org/abs/2608.13547v1
parent_observation_id: null
last_seen_at: 2026-08-14T20:45:21.287190Z
---
...
### 这是什么  
QuoteBench 是一个测评框架，通过在命令生成与执行传输之间加入未转义的解析器，利用精确的最终状态校验来区分模型生成的错误与后处理引入的故障。

### 用在哪里  
适用于评估 LLM 编码代理指令生成链路的研究，尤其是需要了解不同模型配置在序列化、包装和重新解析环境下表现的开发者或评测团队。

### 可以推断的  
- 推测：在执行路径中加入未转义的解析器会把原本成功的命令误判为失败，从而显著降低成功率。  
- 推测：不同的部署配置可能改变模型之间的相对排名，说明评估时必须同时报告生成契约与执行路径的细节。
