...
### 问题**: 在 JeecgBoot 的代码生成器配置中，如何将数据库表中的 `varchar` 类型字段映射为前端组件中的“下拉框”而不是默认的“文本框”？请描述配置步骤。

### 提示**: 关注代码生成器中的“表单配置”或“字段属性设置”，思考除了数据库类型外，哪个属性决定了 UI 组件的类型（例如 `dictCode` 或 `widget_type`）。
