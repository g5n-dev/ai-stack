...
description: 本文介绍了如何通过 Amazon Bedrock AgentCore 的 Policy（策略）功能构建安全的 AI 智能体，主要包含以下核心内容：
  1. **独立的确定性执行层**：Policy 为智能体提供了一个独立于其自身推理逻辑之外的强制执行层，确保业务规则得到严格遵守。 2. **自然语言到代码的转换**：用户
