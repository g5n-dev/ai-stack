...
description: 今天，我们宣布为 Amazon Bedrock 推出两项新的 Amazon CloudWatch 指标： （首字生成时间，TTFT）和 （预估每分钟
  Token 配额使用量）。 这两项新指标旨在提升推理工作负载的运营可见性，帮助用户更好地了解模型性能与配额消耗情况。本文将详细介绍这些指标的工作原理，以及如何利用它们设置
