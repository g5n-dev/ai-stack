...
- 论文
source: arxiv
description: 针对临床脑机接口中瘫痪患者难以提供大规模训练数据的痛点，本文提出了MEG-XL模型，通过引入长达2.5分钟的超长上下文预训练，有效弥补了现有方法忽略长时间神经统计先验的缺陷。实验表明，该模型显著提升了数据效率，仅用少量数据即可达到传统方法需要海量样本才能实现的解码性能。不过，摘要未详细说明其在不同受试者间的泛化能力，具体机制尚无法从摘要确认。
