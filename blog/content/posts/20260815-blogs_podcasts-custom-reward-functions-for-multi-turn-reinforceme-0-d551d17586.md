...
first_seen_at: 2026-08-15T12:59:10.130483Z
...
interpretation_sha256: "sha256:0fba12a37e6a78a20242d9ad41597662aa95a9f1f51ba90c279ffe1c8ce2e9a9"
description: "在 Amazon Nova Forge 的多轮强化学习框架下，作者展示了如何编写并组合奖励函数，以引导模型在多轮交互中学习累积奖励，并演示了如何通过自带编排（BYOO）方式安全执行代码以及监控每个奖励组件的学习贡献。"
...
last_seen_at: 2026-08-15T12:59:10.130483Z
---
...
### 这是什么
在 Amazon Nova Forge 的多轮强化学习框架下，作者展示了如何编写并组合奖励函数，以引导模型在多轮交互中学习累积奖励，并演示了如何通过自带编排（BYOO）方式安全执行代码以及监控每个奖励组件的学习贡献。

### 用在哪里
适用于需要定制多轮对话或代理行为的团队，例如在工具调用、代码执行、错误恢复等场景中，利用强化微调而非传统监督微调来提升模型的分布外泛化能力。

### 可以推断的
推测：在设计复合奖励时，如果某个奖励项在同组的所有样本上取值相同，它将不产生梯度，导致该奖励项实际上没有学习信号。  
推测：由于多轮任务会产生很长的轨迹，单次 Lambda 调用难以完成奖励评估，需要在自有容器环境中运行奖励逻辑并返回聚合分数。
