...
first_seen_at: 2026-08-14T22:39:45.513399Z
...
interpretation_sha256: "sha256:01ab97b71de310d9cff3f7570a8fafc387cd24ca9b1cbfea08b84357fde4d3bc"
description: "这篇内容介绍了在多轮强化学习场景下，如何利用 Amazon Nova Forge 平台设计自定义奖励函数。讲解了奖励函数的运行机制、GRPO 算法的学习方式，以及在自定义环境中处理多轮交互的方法，并分享了实际训练中容易出现的问题及排查思路。"
...
last_seen_at: 2026-08-14T22:39:45.513399Z
---
...
这篇内容介绍了在多轮强化学习场景下，如何利用 Amazon Nova Forge 平台设计自定义奖励函数。讲解了奖励函数的运行机制、GRPO 算法的学习方式，以及在自定义环境中处理多轮交互的方法，并分享了实际训练中容易出现的问题及排查思路。

### 用在哪里

适合需要定制大型语言模型行为、进行强化微调的团队使用。内容针对的是需要在多轮交互任务（如智能体调用工具、执行代码、多轮对话）中训练模型的开发者，特别是那些希望深入控制训练信号、优化模型在复杂任务上表现的技术人员。

### 可以推断的

推测：奖励函数设计中，不同轮次或不同完成结果若得分相同，则无法产生有效的学习信号，导致模型无法从对比中受益。  
推测：与监督微调相比，强化微调在处理分布外任务时更具优势，因为它通过奖励信号而非固定标签进行学习。
