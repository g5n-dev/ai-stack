...
first_seen_at: 2026-08-15T13:44:16.226931Z
...
interpretation_sha256: "sha256:98af28439da9602949352e10aab5efae184f0f032430f55baf83194bc46dc624"
description: "这篇内容介绍如何在多轮强化学习框架下设计复合奖励函数，并配合 GRPO 算法让模型从完整的对话轨迹中学习。"
...
last_seen_at: 2026-08-15T13:44:16.226931Z
---
...
### 这是什么
这篇内容介绍如何在多轮强化学习框架下设计复合奖励函数，并配合 GRPO 算法让模型从完整的对话轨迹中学习。

### 用在哪里
适用于需要在多轮、代理型任务（如调用工具、执行代码）中定制 Amazon Nova 模型行为的团队，尤其是对奖励函数质量有较高要求、愿意自行管理运行环境的开发者。

### 可以推断的
推测：在实际训练中，即使奖励函数的某个组成部分看似重要，如果它在所有候选输出上得分相同，则对学习过程毫无贡献，属于被忽视的失效组件。  
推测：由于多轮对话的评估涉及完整的交互链路而非单一回复，奖励函数的设计需要考虑如何追踪和累计各轮次的状态与结果，以确保最终得分的有效性。
