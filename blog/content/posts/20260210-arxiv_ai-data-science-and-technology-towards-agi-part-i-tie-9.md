...
- 大模型
source: arxiv
description: 本文提出了一种分层级数据管理框架（L0-L4），旨在通过数据与模型的协同进化来推动通用人工智能（AGI）的发展。 背景与观点： 当前大语言模型（LLM）研究过度依赖单纯扩大数据规模的范式，正面临数据可用性、成本和效率的瓶颈。作者认为AGI发展已进入新阶段，即模型应主动引导数据管理，而高质量数据将进一步放大模型能力。
  核心方案： 文章提出从原始资源到组织化知识的五层（L0-L4）管理框架。
