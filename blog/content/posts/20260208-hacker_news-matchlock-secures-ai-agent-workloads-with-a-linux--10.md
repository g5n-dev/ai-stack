...
### 问题**: 在 Linux 环境中，如何使用一条命令快速启动一个受限的 Shell 环境，仅允许特定用户执行 `ls` 和 `pwd` 命令，而禁止其他任何 Shell 操作（如 `cd` 或变量修改）？

### 提示**: 考虑使用 `rbash` (Restricted Bash) 或通过修改用户登录 Shell 的方式来限制环境变量和路径。
