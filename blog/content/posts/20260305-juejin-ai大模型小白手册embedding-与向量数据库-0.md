...
draft: false
entry_kind: "auto"
tags: ["Embedding", "向量数据库", "LLM", "RAG", "语义搜索", "高维向量", "余弦相似度", "AI基础"]
categories: ["大模型", "数据"]
source: juejin
description: "这段内容主要介绍了人工智能领域两个密切相关的基础概念：**Embedding（嵌入）**与**向量数据库**。以下是简洁的总结： **1. 核心概念：Embedding（向量化）** * **作用**：Embedding 是一种将人类可识别的复杂数据（如**文本、图像、音频**等）转化为计算机可处理的**高维向量**（"
...
### 1: 什么是 Embedding（嵌入），为什么大模型需要它？

1: 什么是 Embedding（嵌入），为什么大模型需要它？

**A**: 简单来说，Embedding 是一种将人类语言（文字）转化为计算机能理解的数字列表（向量）的技术。
...
### 2: 向量数据库是什么？它和 MySQL 等传统数据库有什么区别？

2: 向量数据库是什么？它和 MySQL 等传统数据库有什么区别？

**A**: 向量数据库是专门用来存储和检索“向量”数据的数据库。它的核心区别在于检索的方式：
...
### 3: 为什么在做大模型应用（如 RAG）时，一定要用向量数据库？

3: 为什么在做大模型应用（如 RAG）时，一定要用向量数据库？

**A**: 这主要解决了大模型的两个核心痛点：**知识时效性**和**幻觉问题**。
...
### 4: 向量数据库是如何判断两个句子是否相似的？

4: 向量数据库是如何判断两个句子是否相似的？

**A**: 它是通过计算数学上的“距离”来判断的。
...
### 5: 常见的向量数据库有哪些？新手该如何选择？

5: 常见的向量数据库有哪些？新手该如何选择？

**A**: 目前市场上的向量数据库主要分为几类，新手可以根据场景选择：
