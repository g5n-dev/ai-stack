...
draft: false
entry_kind: "auto"
tags: ["RNN", "Transformer", "U(d)", "O(d)", "序列建模", "正交变换", "群论", "架构设计"]
categories: ["论文", "大模型"]
source: arxiv
description: "本文提出了一种基于酉群 $U(d)$ 闭子群的序列建模统一框架。该研究使用极简公理推导出一个共享架构骨架，其中子群的选择直接决定了状态空间、切投影和更新映射，从而自然地导出了循环神经网络（RNN）和Transformer模型。 作者特别在 $O(d)$ 子群上进行了实例化，并在参数匹配的条件下，于 Tiny Shake"
...
### 失败条件
- **数据分布不匹配**：如果数据的生成机制包含显著的**能量放大/衰减**（非酉过程），或者包含剧烈的非线性突变，这种严格的正交约束可能会成为表达的瓶颈，导致模型欠拟合。现实世界的自然语言可能并不严格满足“酉”性质。
