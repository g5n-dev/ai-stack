...
**说明**:
数据代理的行为高度依赖于系统提示词的设计。最佳实践包括为代理分配明确的角色（如“你是一个资深数据分析师”），设定具体的输出格式要求（如 JSON 或 Markdown 表格），以及规定思维链的要求。通过精心设计的结构化指令，可以引导模型更稳定地进行推理，减少无关的闲聊，并确保输出内容能被下游系统直接解析和使用。
