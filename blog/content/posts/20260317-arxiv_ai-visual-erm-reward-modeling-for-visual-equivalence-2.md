...
draft: false
entry_kind: "auto"
tags: ["Visual-ERM", "视觉奖励模型", "LVLM", "Vision-to-Code", "RLHF", "多模态", "cs.CV", "强化学习"]
categories: ["大模型", "论文"]
source: arxiv
...
### 主要结果
1.  **越级表现：** 8B 的 Visual-ERM 在 VC-RewardBench 上超越了 235B 的通用 Qwen 模型，甚至逼近 GPT-4o。证明了专用微调的威力。
2.  **RL 提效：** 将 Visual-ERM 应用于 Qwen3-VL-8B-Instruct 的强化学习训练，在图表生成任务上得分提升 **+8.4**，表格和 SVG 分别提升 **+2.7** 和 **+4.1**。
3.  **测试时增强：** 利用 Visual-ERM 生成的批评文本，指导模型进行自我修正，在零样本设置下也获得了显著性能提升。
...
### 实际应用场景
1.  **智能前端开发：** 直接通过截图生成网页代码，并自动通过 Visual-ERM 进行质量验收。
2.  **数据可视化：** 辅助数据分析师，将 Excel 数据或草图直接转化为出版级的 Matplotlib/Seaborn/ECharts 图表。
3.  **设计工具自动化：** 在 Figma 或 Sketch 中，实现从手绘稿到矢量图（SVG）的自动转化与精修。
...
### 对该领域的启示
1.  **专用 > 通用：** 在特定的高价值领域（如视觉对齐），经过精细数据清洗的中小模型（8B）完全可以在特定任务上击败超大通用模型（235B+）。
2.  **批评的价值：** 未来的 RLHF 不应仅仅追求分数，而应追求“可解释的反馈”。文本反馈是连接感知与认知的桥梁。
...
- **Code-RM (如 CodeReward):** 专注于代码逻辑正确性（能否运行），而 Visual-ERM 专注于代码的**输出结果**（渲染图）是否正确。
...
**A**: Visual-ERM 通常基于 CLIP（Contrastive Language-Image Pre-training）或类似的视觉-语言编码器架构，或者是在 MLLM 基座上添加一个价值头。
