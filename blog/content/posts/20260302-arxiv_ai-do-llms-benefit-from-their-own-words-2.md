...
- 论文
source: arxiv
description: 本文重新审视了多轮对话中保留模型历史回复的常规设计，探究大语言模型（LLM）生成高质量后续内容是否必须依赖自身先前的输出。通过对比“标准全上下文模式”与“仅用户输入模式”，研究旨在剥离模型自生成文本的影响，从而评估其对模型性能的实际增益。鉴于摘要未完整披露具体实验数据与统计显著性，尚无法从摘要确认该设计在不同任务中的具体表现差异。
