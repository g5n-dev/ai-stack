...
draft: false
entry_kind: "auto"
tags: ["MoE", "分布式训练", "通信优化", "LatentMoE", "Head Parallel", "负载均衡", "专家并行", "加速"]
categories: ["大模型", "系统与基础设施"]
source: arxiv
description: "以下是对该内容的中文总结： **背景与问题** 大型语言模型训练成本高昂，稀疏混合专家模型通过条件计算解决了这一问题，其标准的分布式训练方法是专家并行（EP）。然而，EP存在三大局限：通信成本随激活专家数量（$k$）线性增长、负载不均衡影响延迟与显存、以及数据依赖的通信需要元数据交换。 **提出的方案** 针对上述问题"
