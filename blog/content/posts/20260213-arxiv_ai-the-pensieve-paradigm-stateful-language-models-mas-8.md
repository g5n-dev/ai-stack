...
- 论文
source: arxiv
description: 针对大语言模型难以突破固定上下文窗口限制、无法自主管理记忆的问题，本文提出了 StateLM 这一新型基础模型。该研究通过引入内部推理循环及记忆工具，赋予模型动态剪枝与索引上下文的能力，使其从被动接收转变为主动管理自身状态。实验显示，StateLM
  在长文档问答及聊天任务中表现优于标准 LLM，验证了其打破架构限制的潜力。
