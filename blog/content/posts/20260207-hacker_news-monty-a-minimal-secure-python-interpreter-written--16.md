...
categories: ["开发工具", "安全"]
source: hacker_news
description: "随着 AI Agent 对代码执行环境的需求日益增长，传统的 Python 解释器在安全性与资源控制上往往面临挑战。Monty 作为一个由 Rust 编写的极简 Python 解释器，正是为了解决这些痛点而生。它不仅提供了内存安全保障，还能有效隔离执行风险。本文将深入剖析 Monty 的架构设计，展示开发者如何利用它为"
...
### 问题**: Python 的 `globals()` 和 `locals()` 函数允许代码访问和修改作用域内的变量，这在 AI 生成代码时可能导致不可预期的行为或安全漏洞。请设计一个简单的沙箱策略，明确列出哪些 Python 内置函数（如 `open`, `exec`, `eval`, `import`）应该被默认禁用，并解释为什么禁用它们对于 AI 应用场景至关重要。

### 提示**: 考虑文件系统访问、动态代码执行以及模块加载这三个维度的风险点。思考 AI 如果被注入恶意提示词，可能会利用这些函数做什么？
