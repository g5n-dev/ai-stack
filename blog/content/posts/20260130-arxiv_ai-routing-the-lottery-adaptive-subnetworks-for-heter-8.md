...
- 大模型
source: arxiv
description: 针对异构数据场景下传统剪枝方法难以寻找单一“中奖彩票”的局限性，本文提出了“Routing the Lottery”（RTL）这一自适应剪枝框架。该方法通过引入门控机制动态激活子网络，以适应数据的异构分布。虽然摘要未明确披露具体的门控实现细节，无法从摘要确认其计算开销，但该工作为解决非独立同分布数据下的模型压缩提供了新的思路，有望提升边缘设备在复杂环境下的推理效率。
