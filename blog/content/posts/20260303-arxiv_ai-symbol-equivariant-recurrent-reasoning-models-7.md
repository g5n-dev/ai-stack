...
- 大模型
source: arxiv
description: 本文介绍了**符号等变递归推理模型**，这是一种针对数独和ARC-AGI等复杂推理问题的新型神经网络架构。 主要内容包括： 1. **背景与挑战**：现有的递归推理模型（RRM）虽然比大语言模型更紧凑，但通常只能通过昂贵的数据增强来隐式处理符号对称性问题。
  2. **核心创新**：SE-RRM通过在架构层面引入**符号
