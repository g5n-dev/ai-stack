...
- 论文
source: arxiv
description: 针对像素空间计算成本高昂且难以优化的挑战，PixelGen 提出了一种引入感知监督的端到端像素扩散框架。通过结合 LPIPS 与基于 DINO
  的感知损失，该方法在避免 VAE 伪影的同时，有效引导模型聚焦于感知相关的流形，从而在 ImageNet-256 等基准测试中超越了主流潜在扩散模型。这一简洁的范式为提升生成质量提供了新思路，但其在更高分辨率下的计算效率与扩展性尚无法从摘要确认。
