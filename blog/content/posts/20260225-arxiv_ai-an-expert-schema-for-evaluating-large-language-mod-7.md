...
- 数据
source: arxiv
description: '**《学术问答系统中大语言模型错误评估专家模式》内容总结** **背景与问题：** 大型语言模型（LLM）正在改变学术搜索和摘要等任务，但其可靠性仍不确定。目前评估LLM可靠性的主要方法侧重于效率和可扩展性的自动化指标。然而，这些方法缺乏情境细节，无法反映科学领域专家在实际工作中评估LLM输出的方式。
  **研究方法：*'
