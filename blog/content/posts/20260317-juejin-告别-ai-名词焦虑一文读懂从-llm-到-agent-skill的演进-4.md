...
### LLM（大语言模型）和 Agent（智能体）的核心区别是什么？

LLM（如 GPT-4、Claude）本质上是一个基于概率预测的文本生成工具，它的主要能力在于理解上下文、生成内容、逻辑推理和知识问答，但其运行模式是“被动”的——即用户提问，模型回答。
...
### 什么是 Agent Skill（智能体技能），它在演进过程中处于什么位置？

在从 LLM 向 Agent 演进的过程中，单纯的模型能力往往难以精准完成特定领域的垂直任务。Agent Skill 指的是封装好的、特定领域的**能力组件**或**工具集**。
...
### 为什么我们需要从 LLM 进化到 Agent？直接用 ChatGPT 不够吗？

直接使用 ChatGPT 等 LLM 产品存在以下局限性，这也是必须进化到 Agent 架构的原因：
...
### 在 Agent 架构中，RAG（检索增强生成）扮演什么角色？

RAG 是连接 LLM 与私有知识库的桥梁，是 Agent 构建中至关重要的一环。
...
### 开发一个 AI Agent 主要面临哪些技术挑战？

尽管概念很美好，但开发稳健的 Agent 目前仍面临诸多挑战：
