...
2. 将这些函数的逻辑流程、输入输出要求转换为详细的自然语言描述。
...
### 问题**: 定义 "Skill"（技能）与 "Agent"（智能体）在系统架构层面的核心区别。如果将一个简单的 Python 函数封装为 LLM 的工具，它属于哪一类？为什么？

### 提示**: 思考 "静态执行" 与 "动态规划" 的区别。Skill 通常是预先定义好的输入输出映射，而 Agent 是否具备根据环境反馈调整行为的能力？
