...
draft: false
entry_kind: "auto"
tags: ["OpenAI", "CoT", "思维链", "推理模型", "AI安全", "可监控性", "强化学习", "模型对齐"]
categories: ["大模型", "安全"]
source: blogs_podcasts
description: "OpenAI最近推出了一种名为CoT-Control的技术，旨在让用户控制推理模型的思维链（Chain of Thought，CoT）。这项技术的研究发现，推理模型在尝试控制自己的思维链时表现不佳，这反而强化了监控思维链作为AI安全防护措施的重要性。 CoT-Control允许用户通过修改或重写模型生成的思维链来引导推"
...
为了验证文章中关于“CoT 不可控性”及其影响的论断，建议采用以下指标和实验：
...
**如何应用到自己的项目**
1.  **保留原始思维**：在开发基于LLM的应用时，尽量保留并展示模型的原始思维链，而不是直接将其过滤掉。这不仅能增加用户信任，也是最好的调试工具。
2.  **警惕“过度整洁”的思考**：如果你的模型突然开始输出非常工整、像教科书一样但缺乏探索性的思维链，检查是否发生了过拟合或对齐过度。

**具体行动建议**
*   在Prompt设计中，避免使用“请简短思考”或“不要展示犹豫”等可能干扰模型自然推理过程的指令。
*   建立思维链分析流水线，提取思维链中的逻辑连接词和情感倾向，作为模型状态的监控指标。

**注意事项**
思维链中可能包含偏见、错误信息甚至幻觉。直接展示给用户时，需要明确标注“这是模型的内部草稿，不一定准确”，以免误导用户。
...
**实施步骤**:
1. 监控模型输出中的置信度指标或语言信号（如“可能是”、“另一方面”）。
2. 当模型表现出不确定性时，不要强行要求其给出二选一的答案，而是要求其
