...
- 论文
source: arxiv
description: 本文基于“勤奋学习者”框架，探讨了大型语言模型（LLM）通过构建工具提升能力的路径。研究设计了GF(2)电路重建基准，旨在量化模型在测试时搜索中的步骤成功率及分布外推理能力。该工作为评估模型在复杂任务中的工具使用与搜索策略提供了实证视角。
  本文提出了一种通过工具构建实现大型语言模型（LLM）“超级智能”的路径。
