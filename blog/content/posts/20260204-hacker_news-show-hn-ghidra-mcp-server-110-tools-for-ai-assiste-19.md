...
### 问题**: 在 Ghidra 中，当你使用 MCP Server 提供的 `get_current_address` 工具获取当前光标所在的内存地址后，AI 模型通常会返回一个十六进制数值（如 `0x00401050`）。请描述如何利用 MCP 的其他工具（如 `get_functions` 或 `get_symbols`）来验证这个地址是否属于某个已命名的函数或全局符号，并说明这一步在自动化分析中的意义。

### 提示**: 思考如何将一个原始的内存地址映射到 Ghidra 抽象语法树（AST）中的高级节点，以及为什么 AI 需要这种上下文关联才能理解代码逻辑，而不仅仅是处理字节流。
