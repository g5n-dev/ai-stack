...
graph LR
    ...
    B --> C[测试 + 内容门禁]
    C --> D[Hugo + Pagefind 交付构建]
    E[main push] --> F[Build and Deploy]
    G[每小时 :17 / 手动刷新] --> F
    F --> H[采集与历史修复]
    H --> I[图谱 + 趋势派生数据]
    I --> J[Hugo 0.153.4 + Pagefind]
    J --> K[Pages artifact]
    K --> L[GitHub Pages]
    L --> M[ai-stack.site]
```
...
| `.github/workflows/deploy.yml` | `main` push、每小时第 17 分钟、手动 | 定时/手动时刷新内容，所有触发均核验内容、图谱、趋势并上传 Pages artifact；Hugo 固定为 `0.153.4` |
| `.github/workflows/monitoring.yml` | 每 6 小时第 23 分钟、手动 | 比对仓库与生产图谱/趋势的新鲜度，阈值为 12 小时 |

部署不再维护独立发布分支。`actions/upload-pages-artifact` 生成不可变交付物，随后由 `actions/deploy-pages` 发布到 GitHub Pages。`main` push 默认复用已经在 PR 中审核的内容与派生资产；定时任务或显式选择刷新时才执行采集，并通过质量门禁后回写。
...
| `TWITTER_BEARER_TOKEN` | Twitter API (可选) | `Bearer token` |
| `TELEGRAM_BOT_TOKEN` | Telegram Bot (可选) | Bot token |
| `TELEGRAM_CHAT_ID` | Telegram Chat ID (可选) | Chat ID |
...
[params]
  ...
  author = "AI Stack Bot"
  github = "https://github.com/g5n-dev/ai-stack"
