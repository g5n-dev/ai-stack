...
draft: false
entry_kind: "auto"
tags: ["长思维链", "监督微调", "数据重复", "推理能力", "SFT", "AIME", "GPQA", "模型训练"]
categories: ["大模型", "论文"]
source: arxiv
description: "总结：在长思维链监督微调中，数据重复优于数据扩充 这项研究挑战了机器学习中“更多独特训练样本带来更好泛化能力”的传统直觉，发现在针对推理能力的监督微调（SFT）阶段，**数据的重复利用**比单纯的数据量扩充更为有效。 **主要发现与结论：** 1. **重复训练的优势**：在固定的更新预算（计算量）下，使用较小的数据集"
