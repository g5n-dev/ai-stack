...
### SOTAlign 的核心目标是什么？它主要解决了什么技术痛点？

SOTAlign 的核心目标是在半监督学习的框架下，实现单模态视觉模型与语言模型之间的对齐。它主要解决了当前多模态预训练模型（如 CLIP）对大规模且高质量的图文对数据依赖性过强的问题。
...
### SOTAlign 如何利用最优传输理论进行模型对齐？

SOTAlign 将视觉特征和语言特征的分布视为两个不同的概率分布。其核心机制是通过最优传输算法计算这两个分布之间的 Wasserstein 距离，并寻找一个最优传输计划。
...
### 该方法属于全监督学习还是半监督学习？它需要什么样的数据输入？

SOTAlign 属于半监督学习方法。它不需要全量的成对标注数据，而是结合了两种类型的数据输入：
...
### 相比于 CLIP 等传统的对比学习方法，SOTAlign 有何优势？

传统的对比学习方法（如 CLIP）主要依赖大规模的成对数据进行对比学习，其损失函数（如 InfoNCE）关注的是正负样本的相对距离。SOTAlign 的优势主要体现在以下两点：
...
### SOTAlign 的训练流程是怎样的？是否包含预训练阶段？

SOTAlign 的训练流程通常包含两个关键阶段，或者是一个联合优化的过程：
...
### SOTAlign 在实际应用中的表现如何？适用于哪些下游任务？

根据论文在标准基准数据集（如 ImageNet, MSCOCO, Flickr30K）上的实验结果，SOTAlign 在以下方面表现出色：
