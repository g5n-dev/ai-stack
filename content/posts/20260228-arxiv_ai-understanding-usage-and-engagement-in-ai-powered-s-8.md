...
- 论文
source: arxiv
description: '**内容总结：** 本文介绍了**Asta交互数据集**，这是一个包含超过20万条用户查询和交互日志的大规模资源，来源于两个已部署的AI科研工具（基于LLM的文献发现界面和科学问答界面）。
  **主要研究发现包括：** 1. **查询特征：** 用户的查询比传统搜索更长、更复杂，他们将系统视为协作研究伙伴，用于起草内容和'
...
### 什么是 Asta 交互数据集（Asta Interaction Dataset），它的主要研究目的是什么？

Asta 交互数据集是一个专注于科学发现领域的大型交互数据集。它记录了研究人员在使用 AI 驱动的科学工具（具体为名为 Asta 的电子设计自动化工具）时的真实行为数据。
...
### 该数据集中包含哪些类型的数据？它是如何收集的？

该数据集包含了两类主要数据：**日志数据**和**调查数据**。
...
### 根据 Asta 数据集的研究，研究人员在使用 AI 科学工具时的主要行为模式是什么？

通过对 Asta 数据集的分析，研究发现了几个显著的行为模式：
...
### 该研究指出的影响用户“参与度”和“留存率”的关键因素有哪些？

研究表明，影响用户持续使用 AI 科学工具的关键因素主要包括以下几点：
...
### Asta 数据集与通用的 AI 交互数据集（如代码助手或聊天机器人数据集）有何不同？

Asta 数据集的独特之处在于其**领域特殊性**和**任务复杂性**：
...
### 该数据集对于开发未来的 AI 科研工具有什么启示？

Asta 数据集的分析结果为开发者提供了以下重要启示：
...
根据论文来源及 arXiv 的发布惯例，Asta 交互数据集通常会遵循开源科学的精神。研究人员可以通过论文中提到的官方项目页面、GitHub 仓库或相关的数据托管平台（如 Zenodo
