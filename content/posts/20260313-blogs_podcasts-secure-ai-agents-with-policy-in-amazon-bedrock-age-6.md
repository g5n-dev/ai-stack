...
description: 本文介绍了Amazon Bedrock AgentCore中的Policy功能，旨在为AI智能体提供安全防护。核心要点如下： 1. **独立执行层**：Policy创建了一个独立于智能体自身推理之外的确定性执行层，确保安全控制不依赖于模型的不确定性输出。
  2. **自然语言转策略**：用户可以将业务规则的自然语言描述转
