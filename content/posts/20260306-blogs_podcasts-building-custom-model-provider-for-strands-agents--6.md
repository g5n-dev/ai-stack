...
description: 总结：构建自定义模型提供商以集成 SageMaker AI 终端节点与 Strands Agents 这篇文章演示了如何在 AWS SageMaker
  上托管的 LLM（特别是 Llama 3.1）与 Strands Agents 之间建立集成，特别是针对那些不原生支持 Bedrock Messages API
  格式的
