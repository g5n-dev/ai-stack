...
description: AWS近日发布了**大型模型推理（LMI）容器的重要更新**，主要为客户在AWS上托管大语言模型（LLM）带来了以下改进： 1. **全面的性能提升**：针对主流模型架构实现了可衡量的性能增长。
  2. **扩展的模型支持**：扩大了对更多模型的支持范围。 3. **简化的部署能力**：提供了更流畅的部署流程，旨在降低运
