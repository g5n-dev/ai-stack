...
description: '**标题：利用闲置算力，新方法可提升大模型训练效率** **核心内容总结：** 研究人员开发出一种新方法，通过利用**闲置计算时间**（idle
  computing time），在不牺牲模型准确率的前提下，成功将大语言模型（LLM）的训练速度**提升了一倍**。这一突破有望显著提高AI模型训练的效率并降低相关成本。'
