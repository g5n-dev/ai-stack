...
- 数据
source: arxiv
description: 以下是对该内容的中文总结： **CHIMERA：用于提升大模型通用推理能力的紧凑型合成数据集** **背景与挑战** 尽管大语言模型（LLM）在推理能力上表现出色，但在开放和可扩展的环境中复现及提升这些能力，仍面临三大以数据为核心的挑战：
  1. **冷启动问题**：缺乏包含详细、长思维链轨迹的种子数据集，难以初始化推理
