...
description: "这篇文章介绍了 Meta 在构建名为 Prometheus（Prometheus）的千兆瓦级 AI 集群时，后端聚合技术的关键作用。 主要内容总结如下： 1. **核心功能**：BAG 技术使得 Meta 能够无缝连接跨越多个数据中心和区域的数千个 GPU。 2. **网络架构**：该实现连接了两种不同的网络结构——*"
