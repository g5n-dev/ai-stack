...
description: 本文介绍了 Amazon Bedrock AgentCore 中的 Policy（策略）功能，旨在解决 AI 智能体（AI Agents）在执行任务时的安全与控制问题。
  **核心概念与原理** AgentCore 引入了一个**确定性的执行层**，该层独立于智能体自身的推理过程运行。这意味着无论 AI 模型如何生成请求
