...
description: 随着大模型对上下文长度的需求不断增长，如何高效训练超长上下文模型已成为技术攻关的重点。本文详细介绍了 Ulysses 提出的序列并行策略，通过在数据并行维度上分布注意力计算，成功实现了百万级
  Token 上下文的稳定训练。读者将了解到该方案如何在不牺牲训练速度和收敛性的前提下突破显存瓶颈，以及其对长文本应用落地的实际价
